...
This repo is a ComfyUI custom node package. It should live inside your ComfyUI `custom_nodes` folder as `lora_evaluation_project`.

If you are using Stability Matrix, a typical location is:

```text
StabilityMatrix\Data\Packages\ComfyUI\custom_nodes\lora_evaluation_project
```
...
git clone https://github.com/Gyropilot2/lora_evaluation_project.git
cd lora_evaluation_project
```
...
- Confirm the repo lives at `ComfyUI\custom_nodes\lora_evaluation_project`.
- If you are using Stability Matrix, that usually means `StabilityMatrix\Data\Packages\ComfyUI\custom_nodes\lora_evaluation_project`.
